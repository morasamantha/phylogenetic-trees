...
def nj(dmatrix, g, taxas, nombre, act):
    n = len(taxas)
    if n > 2:
        # PASO 1
        # Calculamos R_i
        ...
        g.add_edge(nuevo_vertice, taxa_i, weight=(round(Vi, 2)))
        g.add_edge(nuevo_vertice, taxa_j, weight=(round(Vj, 2)))

        # Seguimos calculandoooooo....
        nj(dist_update, g, taxas, nombre, nuevo_vertice)
    # Paso 4
    elif n == 2:
        # Unimos los dos que quedan, by a branch length d(ti,tj)
        g.add_edge(taxas[0], taxas[1], weight=round(dmatrix[0,1], 2))
        g.remove_node(0)

        plt.figure()    
        pos = nx.planar_layout(g)
        weight_labels = nx.get_edge_attributes(g,'weight')
        nx.draw(g,pos,font_color = 'black', node_shape = 's', with_labels = True, font_size = 11, node_size=200)
...
def neighbor_joining(matrix, headers, nombre):
    ...
    return nj(matrix, G, headers, nombre,0)
